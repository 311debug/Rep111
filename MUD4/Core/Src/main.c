...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of myQueueUART */
  myQueueUARTHandle = osMessageQueueNew (4, sizeof(uint32_t), &myQueueUART_attributes);
...
//extern Model model;
  for(;;)
  {
	...
	//uint8_t byte = 0x99;
	//HAL_UART_Transmit(&huart2, &byte, 1, 10000);
	//osMessageQueuePut(dataQueueHandle, &value, 0, 0); // не блокирует

	//screen1ViewPtr->updateGauge(25);
    //osDelay(100);
  }
  /* USER CODE END 5 */
}
